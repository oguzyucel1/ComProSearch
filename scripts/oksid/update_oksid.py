...
RETRY_DELAY = 5
TIMEOUT = 30
BATCH_SIZE = 500  # Supabase'e batch olarak yazılacak ürünler


# --- Supabase'e Veri Kaydetme ---
def save_to_supabase(products, job_id: str):
    """
    Ürünleri Supabase'e toplu (batch) olarak kaydeder.
    url çakışmalarında güncelleme yapar.
    """
    total = len(products)
...
def fetch_html(url, retries=0):
    """URL'den HTML içeriğini alır, hata durumunda retry yapar"""
    try:
...
def crawl_product_page(initial_url, category_name, job_id):
    """Kategori sayfasını ve alt sayfalarını gezip ürünleri toplar"""
...
def crawl_category(url, job_id, depth=0, visited=None, category_name="Ana Sayfa"):
    """Alt kategorilere recursive girer, ürünleri toplar"""
    if visited is None:
        visited = set()
    if url in visited:
        return
...
def crawl_from_homepage(job_id: str):
    """Ana sayfadan başlayarak tüm kategorileri tarar"""
